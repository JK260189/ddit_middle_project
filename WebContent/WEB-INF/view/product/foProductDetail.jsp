...
<form action="<%= request.getContextPath()%>/purchase/fo_purchase.do" method = "post">
	<input class="purchaseInfo" type = "text" name = "m_code" value = "<%=m_code %>">
	<input class="purchaseInfo" type = "text" name = "fo_code" value = "<%=dVo.getFo_code() %>">
	<input class="purchaseInfo" type = "text" name = "price" value = "">
	<input class="purchaseInfo" type = "text" name = "p_sdate" value = "<%=dVo.getFo_department().substring(0, 10) %>">
	<input class="purchaseInfo" type = "text" name = "p_edate" value = "<%=dVo.getFo_arrival().substring(0, 10) %>">
	<input class="purchaseInfo" type = "text" name = "fo_name" value = "<%=dVo.getFo_name() %>">
	...
	<tr>	
		<td colspan = '2'>
		<h4><%=dVo.getFo_name() %></h4>
		</td>
	</tr>
	<tr>
		<td>상품 소개</td>
		<td>
		<%=dVo.getFo_content().replace("\r\n", "<br>") %>
		</td>
	</tr>
	...
	<tr>
		<td>출발 / 도착지</td>
		<td><%=dVo.getDep_loca1() %> / <%=dVo.getDep_loca2() %></td>
	</tr>
	...
	<tr>
		<td>출발날짜</td>
		<td><%=dVo.getFo_department().substring(0, 10) %> / </td>
	</tr>
	
	<tr>
		<td>도착날짜</td>
		<td><%=dVo.getFo_arrival().substring(0, 10) %> / </td>
	</tr>
	
	<tr>
		<td>가격</td>
		<td class = "selectPrice" value = "<%=dVo.getFo_price() %>" ><%=dVo.getFo_price() %>원</td>
	</tr>
		
	<tr>
		<td>수량</td>
		<td class = "quantity" data-unitprice= "<%=dVo.getFo_price()%>">
